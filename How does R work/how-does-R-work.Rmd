...
4. E.g,. `iris` refers to a `data.frame` (dataset), and is included with `R`'s `base` package.
...
4.  The parantheses contain the arguments (i.e,. input) of the function.
4.  We'll look closer at functions later on, but here is an example using the `head()` function, to show the first 5 rows of the `iris` dataset.
