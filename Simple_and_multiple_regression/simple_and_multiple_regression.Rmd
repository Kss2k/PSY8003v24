...
# A little tip: 
  ...
  # Note that some packages assume that you're using positional arguments
  # E.g., margins

reg_price_size <- df %>% lm(data =, #%>% pipes to the first argument
                            formula = flat_price ~ floor_size)
# Another alternative is to attach the dataframe-object to R's search path first
# In general this is not recommended if your're working with more than one dataset at a time

attach(df)

reg_price_size <- lm(flat_price ~ floor_size) # We no longer need to specify data
