...
1.  Today we will be talking about how R works
2.  Before we do anything we will have a quick tour of Rstudio (note you might have your panes in different locations)
    1.  The Console Pane
        1.  Console (the most important), Terminal etc
        2.  The Console and Terminal both have command lines
    2.  The Viewer-pane
        1.  Here you can view plots, files, folders, working directory etc
    3.  The Environment-pane
        1.  Here you can see the objects and functions loaded into your global environment
    4.  The Script-pane
        1.  Here you can use different type of scriptfiles for R (and other languages: python, C++ etc)
        2.  Right now I am using an R notebook
        3.  But usually i recommend using R-scripts

# Section 1: The R-language

## Objects and Functions

Almost every expression in R can be conceptualized in terms of creating- or using functions, objects or both.

# Objects

1.  Objects are simply named containers of information
2.  On a general level we can distinguish between different types of containers and different types of information
3.  Objects are named in R using the "\<-" (and "-\>") operator
...
# Data Types
## Vectors 
1. The most important data type in R, is the vector. 
2. In essence, everything in R is a vector. 
3. A vector is simply "list" of different values of a particular type of data. 
4. For example, datasets consist of multiple columns, where each column is a vector

### Numeric (ratio/scale, depends on use)

1.  Real numbers (i.e., numbers with decimals)
...
3. To combine multiple vectors into a single vector, we can use the c() command.
...
3. In practice both [] and [[]] do the same thing, where [] can access multiple 
   values at the same time, whilst [[]] only access a single value.
...
2. Dataframes are essentially just a collection of vectors (stored in columns) with an 
   equal number of rows.
3. There are multiple ways of accessing data in dataframes. 

```{r}
df <- read.csv("ChickWeight.csv") # Here we use the read.csv function to read a csv-file and store it in an object
...
4. There are also more complicated ways of subsetting data for dataframes, 
   using logical vectors, and a different notation (matrix-notation). 
...
3.  Depending on your operating system, version of R-studio, what type of file 
 you are using, etc, you might need to change your working directory to the 
...
2.  Open the "ChickWeight.csv" file, and store it as "df"
3.  View the dataset using the "View()" function tip: use help("View")
4.  Find the mean of the weight variable
5.  Find the sd of the weight variable (hint: help(sd))
6.  Find out what the "table()" function does, and use it
7.  Find out what the "summary()" function does, and use it
8.  Find out what the "str()" function does, and use it
...
df %>% lm(data = ,formula = weight ~ Diet) %>% pprint::preg()
